...
import { cancelFutureInstallments, createInstallmentPlan, deleteCreditCard, deleteCreditCardActivity, updateCreditCard, updateCreditCardPaymentOwner, updateInstallmentClassification } from "../actions";
  ...
  return (
    <main className="main">
      ...
      <section className="panel" style={{ marginTop: 16 }}>
        ...
        {postedActivity.length === 0 ? <div className="empty">No posted card activity yet.</div> : <div className="table-wrap"><table><thead><tr><th>Date</th><th>Description</th><th>Type</th><th>Category / Account</th><th>Owner / Paid by</th><th>Amount</th><th></th></tr></thead><tbody>{postedActivity.map((row) => {
          const impact = creditCardImpact(row.activity_type, row.amount);
          const secondary = row.activity_type === "payment" ? accountMap.get(row.account_id ?? "") ?? "Payment account" : categoryMap.get(row.category_id ?? "") ?? "—";
          return <tr key={row.id}><td>{row.activity_date}</td><td>{row.description || activityLabel(row.activity_type)}</td><td><span className={`cc-activity-badge cc-${row.activity_type}`}>{activityLabel(row.activity_type)}</span></td><td>{secondary}</td><td>{row.activity_type === "payment" ? <form action={updateCreditCardPaymentOwner} className="cc-payment-owner-form"><input type="hidden" name="credit_card_id" value={card.id} /><input type="hidden" name="id" value={row.id} /><select name="owner_id" defaultValue={row.owner_id ?? ""} aria-label="Paid by"><option value="">Unassigned</option>{(owners ?? []).map((owner) => <option key={owner.id} value={owner.id}>{owner.name}</option>)}</select><button className="text-btn" type="submit">Save</button></form> : row.owner_id ? ownerMap.get(row.owner_id) ?? "—" : "—"}</td><td className={`${impact < 0 ? "positive" : "negative"} amount-cell`}>{impact < 0 ? "−" : "+"}{money(Math.abs(impact))}</td><td>{!row.installment_id && <form action={deleteCreditCardActivity}><input type="hidden" name="credit_card_id" value={card.id} /><input type="hidden" name="id" value={row.id} /><button className="icon-btn danger-icon-btn" title="Delete activity" type="submit">×</button></form>}</td></tr>;
        })}</tbody></table></div>}
